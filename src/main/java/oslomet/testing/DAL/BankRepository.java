...
public class BankRepository {
  ...
  public Konto hentTransaksjoner(
    String kontonr,
    String fraDato,
    String tilDato
  ) {
    ...
    try {
      String sql =
        "SELECT * FROM Transaksjon WHERE Kontonummer = ? AND Dato >= ? " +
        "AND Dato <= ? AND Avventer = 0";

      List<Transaksjon> transaksjoner = db.query(
        sql,
        new BeanPropertyRowMapper(Transaksjon.class),
        kontonr,
        fraDato,
        tilDato
      );
      ...
      List<Konto> konti = db.query(
        sql,
        new BeanPropertyRowMapper(Konto.class),
        kontonr
      );
  ...
  public String sjekkLoggInn(String personnummer, String passord) {
    try {
      String sql =
        "Select count(*) from Kunde Where personnummer = ? AND passord = ?";
      int funnetBruker = db.queryForObject(
        sql,
        Integer.class,
        personnummer,
        passord
  ...
  public List<Konto> hentKonti(String personnummer) {
    try {
      String sql =
        "Select Distinct Kontonummer from Konto Where Personnummer = ?";
      List<Konto> konti = db.query(
        sql,
        new BeanPropertyRowMapper(Konto.class),
        personnummer
  ...
  public String registrerBetaling(Transaksjon betaling) {
    try {
      String sql =
        "Insert into Transaksjon (FraTilKontonummer,Belop,Dato,Melding,Kontonummer,Avventer) " +
        "Values (?,?,?,?,?,'1')";
      db.update(
        sql,
        betaling.getFraTilKontonummer(),
        betaling.getBelop(),
        betaling.getDato(),
        betaling.getMelding(),
        betaling.getKontonummer()
      );
      return "OK";
    } catch (Exception e) {
      return "Feil";
  ...
  public List<Transaksjon> hentBetalinger(String personnummer) {
    ...
    try {
      String sql =
        "Select * from Transaksjon Join Konto On Transaksjon.Kontonummer = Konto.Kontonummer " +
  ...
  public Kunde hentKundeInfo(String personnummer) {
    try {
      String sql =
        "Select * from Kunde Join Poststed On Kunde.Postnr = Poststed.Postnr Where Personnummer= ?";
      List<Kunde> kunder = db.query(
        sql,
  ...
  public String endreKundeInfo(Kunde kunde) {
    ...
    try {
      sql =
        "Update Kunde Set Fornavn = ?, Etternavn = ?," +
