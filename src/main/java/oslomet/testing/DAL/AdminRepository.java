...
public class AdminRepository {
  ...
  public List<Kunde> hentAlleKunder() {
    try {
      String sql =
        "Select * from Kunde Join Poststed On Kunde.Postnr = Poststed.Postnr ORDER BY Personnummer";
      List<Kunde> alleKunder = db.query(
        sql,
  ...
  public String registrerKonto(Konto konto) {
    try {
      ...
      int funnetPersonnummer = db.queryForObject(
        sql,
        Integer.class,
        konto.getPersonnummer()
      );
      if (funnetPersonnummer == 0) {
        return "Feil";
      }
      sql =
        "Insert into Konto (Personnummer, Kontonummer, Saldo, Type, Valuta) Values (?,?,?,?,?)";
      db.update(
        sql,
        konto.getPersonnummer(),
  ...
  public String endreKonto(Konto konto) {
    try {
      ...
      int funnetKontonummer = db.queryForObject(
        sql,
        Integer.class,
        konto.getKontonummer()
      );
      ...
      sql =
        "Update Konto Set Personnummer = ?, Kontonummer = ?, Type = ?, " +
